...
"""
Fixed Local Server with Canvas-Based Secure Image Rendering
Corrected for Peewee ORM (no db_session)
"""
import os
import socket
import uuid
...
class ImprovedLocalServer:
    ...
    def setup_routes(self):
        ...
        async def serve_photo(photo_id: int, session: str):
            ...
            # Verify student has access to this photo
            student_photo = StudentPhoto.get_or_none(
                (StudentPhoto.student_id == session_data['student_id']) &
                (StudentPhoto.photo_id == photo_id)
            )
        ...
        async def download_photo(photo_id: int, session: str):
            ...
            # Track download
            photo_download = PhotoDownload.create(
                share_session_uuid=session,
                student_id=session_data['student_id'],
                photo_id=photo_id,
                downloaded_at=datetime.utcnow()
            ...
            # Update student total downloads
            student = Student.get_or_none(Student.id == session_data['student_id'])
            if student:
                student.total_downloads = (student.total_downloads or 0) + 1
                student.save()
